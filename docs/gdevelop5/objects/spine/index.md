...
It uses files exported from Spine (a skeleton as a `.json` or binary `.skel` file, an `.atlas`, and image files) to render and animate characters or elements with smooth, interpolated bone-based movement.
...
| ------------------- | --------------------------------------------------------------------------------------------------------- |
| **Spine JSON file** | The main file describing bones, animations, and attachments. A binary skeleton (`.skel`) exported from Spine can be used instead of the `.json` file. |
