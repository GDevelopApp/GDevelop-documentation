...
It uses files exported from Spine (a `.json` or a binary `.skel` skeleton file, a `.atlas` file, and image files) to render and animate characters or elements with smooth, interpolated bone-based movement.
...
| ------------------- | --------------------------------------------------------------------------------------------------------- |
| **Spine JSON file** | The main file describing bones, animations, and attachments. Both the text `.json` and the binary `.skel` skeleton formats are supported. |
