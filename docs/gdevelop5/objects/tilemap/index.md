...
!!! tip
    ...
    The Tilemap object can generate its own collisions: enable the **Enable collision** property in its Collision properties. Use the **Class filter** property to only create hitboxes for tiles with a given "Class" (set in Tiled). This avoids creating a separate collision mask object when a single kind of collision is enough. When you need several kinds of collisions (platforms, ladders...) on the same map, use the dedicated collision mask object described below.
...
Like the [integrated Tilemap object](/gdevelop5/objects/simple_tilemap), the External Tilemap can be modified while the game is running: you can read, set, remove or flip tiles (using scene or grid coordinates) and change the number of rows and columns of the map with actions and expressions. This is useful to build or alter levels dynamically.
