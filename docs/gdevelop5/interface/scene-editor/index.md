...
At the top of the panel, next to the instance lock, a visibility toggle lets you make the selected instance **hidden when the scene starts**. The instance still exists and reacts to collisions and events: it's simply not displayed until you show it (for example with the *Show* action). Hidden instances are shown with a dedicated icon in the scene editor so you can still find and select them.
