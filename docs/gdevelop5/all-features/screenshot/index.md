...
!!! warning
    ...
    Saving a screenshot to a file only works when the game is running on **Windows, Linux or macOS** (desktop). It has no effect on web games or mobile (Android/iOS), because those platforms don't allow writing files directly to the device.
...
If the path does not end with `.png`, the extension automatically adds it, so the screenshot is always saved as a PNG image.
