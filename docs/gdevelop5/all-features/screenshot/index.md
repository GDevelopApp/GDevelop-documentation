...
!!! warning
    ...
    Taking a screenshot is only supported when the game runs as a **desktop application** (Windows, Linux or macOS export). It has no effect in games running in a browser (web export) or on mobile, as those platforms don't allow writing files directly to the device.
