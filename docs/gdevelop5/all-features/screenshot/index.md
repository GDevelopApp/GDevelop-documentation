...
!!! warning
    ...
    Taking a screenshot is only supported when the game runs as a **desktop application** (Windows, Linux or macOS). It does nothing when the game runs on the **web** or on **mobile**, because those platforms don't allow writing files directly to the file system.
...
Relative paths are not supported. If the path doesn't already end with `.png`, the extension automatically appends it, so the file is always saved as a PNG image.
