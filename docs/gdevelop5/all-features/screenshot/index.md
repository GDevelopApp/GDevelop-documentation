...
!!! warning
    ...
    Taking a screenshot writes a file directly to the computer's file system, so it only works in games exported for **Windows, Linux and macOS** (desktop). It has no effect in the web export or on mobile (Android/iOS).
...
If the path does not already end with `.png`, the extension is added automatically, so `MyScreenshot` is saved as `MyScreenshot.png`.
