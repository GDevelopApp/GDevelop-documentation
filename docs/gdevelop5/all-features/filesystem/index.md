...
####  Read a directory
This action reads the contents of a directory (all files and sub-directories) and stores them in a variable, as an array. This is useful, for example, to list all the save files a player has created in a folder.
== Parameters ==
** Directory path: ** The absolute path to the directory to read.
It is advised to use the expressions for special folders (see below) to keep your game platform independent.

** (Optional) Variable to store the result: ** The variable is filled with an array containing the name of every file and sub-directory found in the directory. If an error occurs, it is set to `"error"` instead.
...
####  User's Home folder
This expression returns the operating system independent path to the current user's home folder (for example `C:\Users\Name` on Windows or `/home/name` on Linux).
####  Pictures folder
...
The following expressions take an existing path and extract a part of it. They are handy to display a file name to the player or to rebuild a path from an existing one.
...
This expression returns the name of the file, with its extension if it has one.
