...
####  Read a directory
This action reads the contents of a directory (all files and sub-directories) and stores their names in a variable, as an array. This is useful to discover files created by the player or generated by the game (for example, to list all available save files). This action runs __asynchronously__.
== Parameters ==
** Directory path: ** The absolute path to the directory to read.
It is advised to use the expressions for special folders (see below) to keep your game platform independent.

** Variable to store the result: ** The variable is filled with an array containing the name of each file and sub-directory found. It is set to 'error' instead if the directory could not be read.
...
This expression returns the full path to the game executable file itself (including the file name), rather than just the folder that contains it.
####  User's Home folder
This expression returns the operating system independent path to the current user's home folder (for example _C:\Users\Name_ on Windows or _/home/name_ on Linux).
####  Pictures folder
...
This expression takes a full path and returns only the directory portion of it, without the ending file name. Useful to extract the folder a file lives in.
####  Get file name from a path
This expression takes a full path and returns only the file name (including its extension, if any), without the leading directories.
####  Get the extension from a file path
This expression returns the extension of the file designated by the given path, including the leading period (for example ".txt"). Useful to check the type of a file returned when reading a directory.
