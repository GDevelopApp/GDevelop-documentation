...
####  Read a directory
This action reads the contents of a directory (all files and sub-directories it contains) and stores them in a scene variable as an array. It always runs __asynchronously__.
== Parameters ==
** Directory path: ** The path on the file system of the directory to read.
It is advised to use the expressions for special folders (see below) to keep your game platform independent.

** (Optional) Variable to store the result: ** The variable is set to `"error"` if the directory could not be read, otherwise it is filled with an array containing the name of every file and sub-directory found in the directory.
...
This expression returns the operating system independent path to the game executable file itself (including the file name).
...
The following expressions extract a specific part of an existing path, which is useful when you need to work with a file the player selected or a path stored in a variable:

####  Get directory name from a path
Returns the portion of the given path that represents the directories, without the ending file name.
####  Get file name from a path
Returns the name of the file (with its extension, if any) at the end of the given path.
