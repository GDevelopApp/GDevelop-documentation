...
####  Read a directory
This action reads the contents of a directory (all files and sub-directories) and stores their names in a scene variable, as an array.
== Parameters ==
** Directory path: ** The absolute path to the directory to read.
It is advised to use the expressions for special folders (see below) to keep your game platform independent.

** (Optional) Variable to store the result: ** The variable is set to the value 'error' if an error occurred, otherwise it is filled with an array containing the name of each file and sub-directory found in the directory.

!!! note

    The load actions above have an optional __Normalize the file content__ parameter (enabled by default). When enabled, Windows new line characters ("CRLF") are replaced by a single new line character, which makes text loaded from files consistent across operating systems.
...
This expression returns the operating system independent path to your game's executable file itself (not just the folder containing it).
####  User's Home folder
This expression returns the operating system independent path to the current user's home folder.
####  Pictures folder
...
The following expressions help you extract parts of an existing path (for example, a path returned by the *Read a directory* action):

####  Get directory name from a path
This expression returns the portion of a path that represents the directories, without the ending file name.
####  Get file name from a path
This expression returns the name of the file (with its extension, if any) from a path.
