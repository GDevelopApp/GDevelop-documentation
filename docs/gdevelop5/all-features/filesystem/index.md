...
####  Read a directory
This action reads the contents of a directory (all files and sub-directories) and stores their names in an array. This action is always asynchronous.
== Parameters ==
** Directory path: ** The path on the file system of the directory to read.
It is advised to use the expressions for special folders (see below) to keep your game platform independent.

** (Optional) Result variable: ** Variable to store the result. It is set to `'error'` if an error occurred, otherwise it is filled with an array containing the name of each file and sub-directory found in the directory. You can loop through this array with a "For each child" event to process every entry.
...
This expression returns the operating system independent path to your game's executable file.
...
The following expressions take an existing path and extract a part of it. They are useful, for example, to process the entries returned by the *Read a directory* action.
