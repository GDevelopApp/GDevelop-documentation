...
!!! tip
    ...
    Links between objects are **saved and restored** by the [Save State extension](/gdevelop5/all-features/save-state). Objects excluded from the save state (marked as "Do not save") keep their links to other non-saved objects, but any link between a saved and a non-saved object is not persisted.
