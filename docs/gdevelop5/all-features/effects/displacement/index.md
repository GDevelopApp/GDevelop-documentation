...
You can use this effect to apply all manner of warping effects. Currently, the `r` (red) property of the texture is used to offset the x-axis and the `g` (green) property of the texture is used to offset the y axis. The **Scale on X axis** and **Scale on Y axis** settings control how strong the displacement is on each axis: higher values push pixels further, and you can set them independently to warp more in one direction.
